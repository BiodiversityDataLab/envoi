...
try:
    from .helpers import (
        ...
        DatasetSelection,
        build_run_config,
        default_statistics_for_dataset,
        normalize_crs,
        parse_window_sizes,
...
CONTENT_MARGIN_LEFT_REM = 5.5
CONTENT_MARGIN_RIGHT_REM = 8.0
MOBILE_CONTENT_MARGIN_REM = 1.25
...
def _ensure_dataset_state(default_dataset: str | None) -> None:
    st = _load_streamlit()
...
def _render_header(st) -> None:
    ...
    if logo is not None:
        cols[0].image(str(logo), width=86)
    with cols[1]:
        st.markdown(
            """
...
def _render_dataset_rows(st, catalog: dict[str, dict[str, Any]], output_type: str) -> None:
    ...
    _ensure_dataset_state(dataset_names[0])

    for index, row in enumerate(st.session_state.dataset_rows):
        with st.expander(f"Dataset {index + 1}", expanded=True):
            top_cols = st.columns([0.52, 0.28, 0.2], vertical_alignment="bottom")
            current_dataset = (
                row.get("dataset") if row.get("dataset") in dataset_names else dataset_names[0]
            )
            selected_dataset = top_cols[0].selectbox(
                "Dataset",
                dataset_names,
                index=dataset_names.index(current_dataset),
                key=f"dataset_{index}",
            )
            windows = top_cols[1].text_input(
                "Window size(s), metres",
                value=row.get("window_sizes", "500"),
                key=f"windows_{index}",
            )
            if top_cols[2].button(
                "Remove", key=f"remove_{index}", disabled=len(st.session_state.dataset_rows) == 1
            ):
                st.session_state.dataset_rows.pop(index)
                st.rerun()

            st.session_state.dataset_rows[index]["dataset"] = selected_dataset
            st.session_state.dataset_rows[index]["window_sizes"] = windows

            if output_type == TABULAR_OUTPUT:
                if row.get("statistics_dataset") == selected_dataset:
                    defaults = list(row.get("statistics") or [])
                else:
                    defaults = list(default_statistics_for_dataset(catalog[selected_dataset]))
                valid_defaults = [stat for stat in defaults if stat in reducers]
                selected_stats = st.multiselect(
                    "Spatial statistics",
                    reducers,
                    default=valid_defaults,
                    key=f"stats_{index}_{selected_dataset}",
                )
                st.session_state.dataset_rows[index]["statistics"] = selected_stats
    ...
    if st.button("Add another dataset"):
        st.session_state.dataset_rows.append(
            {"dataset": dataset_names[0], "window_sizes": "500", "statistics": []}
        )
        st.rerun()
...
def _render_footer(st) -> None:
    st.markdown(
        ...
        <div class="envoi-footer">
          envoi is developed at the Biodiversity Data Lab at Uppsala University by
          Adrian Baggstrom and Jakob Nystrom, with support from Tobias Andermann.
          The work is supported by the SciLifeLab and Wallenberg Data Driven Life
          Science Program and the Swedish Research Council. Links:
...
def render_app() -> None:
    ...
    st.write(
        "Upload a CSV with GBIF-style columns: gbifID, decimalLatitude, decimalLongitude, and optionally eventDate."
    )
    uploaded_csv = st.file_uploader("Sampling CSV", type=["csv"])
    ...
    try:
        ...
        st.caption(f"Input CRS: {normalized_crs}")
    ...
    credentials_file = st.file_uploader("Earth Engine service account key JSON", type=["json"])
    credentials_bytes = credentials_file.getvalue() if credentials_file is not None else None

    st.subheader("3. Add data products")
    st.write("Add one row per Earth Engine data product. Each row becomes one envoi output batch.")
