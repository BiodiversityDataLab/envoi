# Combined review: architecture + hands-on findings

**Purpose:** This file merges [future-development-review.md](future-development-review.md) (architectural review for the two planned development arcs — multi-provider integrations and a public Streamlit service) with [MR_report.md](MR_report.md) (hands-on findings from installing and running Envoi) into one document. A priority table is suggested. Full original content of both source documents follows unchanged.

## Priority overview

| ID | Source | Item | Type | Suggested priority | Why |
|----|--------|------|------|---------------------|-----|
| MR-6 | MR_report | Categorical NA regex bug (`_output_assembly.py:32`, `lulc_naturallands` bands silently produce empty cells) | Bug | Critical | Produces silently wrong output today, for any current single-user run — not contingent on the hosting/multi-provider roadmap. |
| F1 | future-development-review | Earth Engine credential contexts are process-global | Architecture / security | Critical (blocks public hosting) | Concurrent users can overwrite or run under another user's credentials. |
| F2 | future-development-review | Hosted output paths are unsafe and don't deliver files to users | Architecture / security | Critical (blocks public hosting) | Server-local, collision-prone, no delivery mechanism. |
| F3 | future-development-review | No admission control or job isolation | Architecture / security | Critical (blocks public hosting) | Unbounded workers/uploads can exhaust shared resources. |
| MR-5 | MR_report | Redundant `lat_original`/`lon_original` columns; wgs84 columns placed at end (`extract.py:422+`) | Bug / cleanup | High | Confirmed by user, has a working patch attached already. |
| F4 | future-development-review | Adapter interface incomplete and unenforced | Architecture | High | Needed before adding CHELSA/Copernicus adapters. |
| F5 | future-development-review | Generic config parsing contains Earth Engine-specific policy | Architecture | High | Same — blocks clean multi-provider support. |
| F6 | future-development-review | Catalog schema assumes raster-like path for every source | Architecture | High | Same. |
| F7 | future-development-review | Adapter result dictionaries form an undocumented internal API | Architecture | High | Same. |
| F8 | future-development-review | Operational failures conflated with valid no-data results | Architecture / correctness | High | Affects data trustworthiness, not just extensibility. |
| F9 | future-development-review | Eager provider imports will complicate optional integrations | Architecture | High | Needed before adding provider SDKs as dependencies. |
| F10 | future-development-review | No generic job-scoped execution context | Architecture | High | Prerequisite for credential/job isolation (ties to F1). |
| MR-4 | MR_report | Webapp forces one data-product type (tabular or raster) per run | Feature request | Medium | Usability improvement for the hosted app; not blocking. |
| F11 | future-development-review | Catalog registration is process-global | Architecture | Medium | Multi-user risk, smaller blast radius than F1–F3. |
| F12 | future-development-review | Adapter lifetime repeats setup across windows | Architecture / performance | Medium | Efficiency, not correctness or safety. |
| F13 | future-development-review | Reducer definitions and semantics are distributed | Architecture | Medium | Maintainability. |
| F14 | future-development-review | Webapp duplicates and imports private core policy | Architecture | Medium | Maintainability / drift risk. |
| F15 | future-development-review | Core output is filesystem-first and weakly typed | Architecture | Medium | Needed for object-storage delivery (ties to F2). |
| F16 | future-development-review | Config accepts silent typos, no schema versioning | Architecture | Medium | Robustness. |
| F17 | future-development-review | Production deployment and service tests absent | Architecture / testing | Medium | Needed before any public deployment, but sequenced after F1–F3 decisions. |
| MR-1 | MR_report | README: pip install fails on macOS 13 (pyproj/rasterio wheels) | Docs | Low | Quick doc fix; consider a GitHub issue to point users to. |
| MR-2 | MR_report | README: missing code snippets for 3 of 4 credential placement options | Docs | Low | Quick doc fix. |
| MR-3 | MR_report | README: unclear which code chunks are bash vs. python | Docs | Low | Quick doc fix. |

---

## Part A — Envoi future-development architecture review

*(full content of [future-development-review.md](future-development-review.md), unchanged)*
...
## Part B — MR hands-on findings

*(full content of [MR_report.md](MR_report.md), unchanged)*
