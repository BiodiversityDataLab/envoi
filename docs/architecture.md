...
| `catalog_docs.py` | Renders catalog entries as a formatted Markdown document (theme sections, summary tables, per-dataset detail blocks with bands, licence, and citation). Exposed as `catalog_markdown()` for notebooks; `scripts/generate_dataset_docs.py` uses it to regenerate `docs/datasets.md`. |
